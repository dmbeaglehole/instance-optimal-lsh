...
void Dataset::read_ImageNet(std::string file_path) 
{
    // HARD-CODED Dimensions
    int number_of_images = LARGE_SIZE;
    int n_cols = 192;
    int n_rows = 1;
    ...
    if (file.is_open())
    {
        ...
        std::cout << "Throwing user exception in ImageNet" << std::endl;
...
Dataset* Dataset::hash_dataset(std::vector<HashFunc> hashes) {
...
*/
    ...
    for (int i=0;i<num_images;i++) {
        ...
        for (unsigned int j=0; j<hashes.size(); j++) {
            HashFunc hashf = hashes.at(j);
            //std::cout << hashf.bit;
            //std::cout << ",";
            //std::cout << hashf.idx << std::endl;
