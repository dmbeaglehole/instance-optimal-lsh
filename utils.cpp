...
void LSHtree::append_hash(std::vector<HashFunc> new_hashes) {
    used_hashes.push_back(new_hashes);
}

LSHtree* LSHtree::add_hashes(std::vector<HashFunc> new_hashes) {
    LSHtree* new_tree_ptr = new LSHtree;
    new_tree_ptr->used_hashes = used_hashes;
    new_tree_ptr->append_hash(new_hashes);

    children.push_back(new_tree_ptr);
    return new_tree_ptr; 
}

std::vector<std::vector<HashFunc>> LSHtree::get_used_hashes() {
    return used_hashes;
}

std::vector<LSHtree*> LSHtree::get_children() {
    return children;
}
...
void LSHtree::DestroyRecursive(LSHtree* tree){
    if (tree) {
        for (unsigned int i=0; i<tree->children.size();i++) {
            LSHtree::DestroyRecursive(tree->children.at(i));
        }
        delete tree;
    }
}
...
LSHtree::~LSHtree(){
    for (unsigned int i=0; i<children.size();i++) {
        LSHtree::DestroyRecursive(children.at(i));
    }
}
